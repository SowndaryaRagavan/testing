...
const BACKEND_URL = "http://127.0.0.1:8000/demo-projects";
...
const TypeScriptDemo: React.FC = () => {
  ...
  const handleAddProject = async () => {
    ...
    try {
      const res = await axios.post<DemoProject>(`${BACKEND_URL}/upload`, formData, {
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 40px", backgroundColor: darkMode ? "#1a1a1a" : "#6c5ce7" }}>
        <button
          onClick={() => navigate("/")}
          style={{ padding: "8px 16px", borderRadius: 6, backgroundColor: "#ffffff", color: "#6c5ce7", fontWeight: 600, cursor: "pointer" }}
        >
          Back to Home
        </button>
        <button
      ...
      <section style={{ padding: "40px", minHeight: "100vh", backgroundColor: darkMode ? "#121212" : "#e7e3ff", color: darkMode ? "#fff" : "#1a1a1a" }}>
        ...
        <div style={{ display: "flex", gap: "20px", justifyContent: "center", marginBottom: "30px" }}>
          <button
            onClick={() => {
              ...
              if (!showTable) fetchProjects();
            }}
            style={{ padding: "10px 20px", borderRadius: 8, backgroundColor: "#6c5ce7", color: "#fff", fontWeight: 600, cursor: "pointer" }}
          >
            {showTable ? "Hide Projects" : "Show Projects"}
          </button>
          <button
            onClick={() => setShowAddModal(true)}
            style={{ padding: "10px 20px", borderRadius: 8, backgroundColor: "#00b894", color: "#fff", fontWeight: 600, cursor: "pointer" }}
          >
            Add Project
          </button>
        </div>
        ...
        {showAddModal && (
          <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", backgroundColor: "rgba(0,0,0,0.5)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 999 }}>
            <div style={{ backgroundColor: darkMode ? "#1f1f1f" : "#ffffff", padding: 30, borderRadius: 12, minWidth: 320, boxShadow: darkMode ? "0 6px 18px rgba(0,0,0,0.35)" : "0 6px 18px rgba(0,0,0,0.08)" }}>
              <h3>Add Project</h3>
              <input
                type="text"
                placeholder="Title"
              ...
              <input
                type="file"
                onChange={(e) => setNewProject({ ...newProject, file: e.target.files ? e.target.files[0] : null })}
                style={{ marginBottom: 10 }}
              />
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <button
                  onClick={handleAddProject}
                  style={{ padding: "8px 16px", borderRadius: 6, backgroundColor: "#00b894", color: "#fff", fontWeight: 600, cursor: "pointer" }}
                >
                  Submit
                </button>
                <button
                  onClick={() => setShowAddModal(false)}
                  style={{ padding: "8px 16px", borderRadius: 6, backgroundColor: "#d63031", color: "#fff", fontWeight: 600, cursor: "pointer" }}
                >
                  Clear
                </button>
              </div>
        ...
        {/* Animated Table */}
        {showTable && (
          <div style={{ maxWidth: 1000, margin: "0 auto", overflowX: "auto", transition: "all 0.5s ease-in-out" }}>
              ...
              <tbody>
                {projects.map((p, i) => (
                  <tr
                    key={p.id}
                    style={{
                      backgroundColor: i % 2 === 0 ? (darkMode ? "#121212" : "#f0f0f0") : "transparent",
                      opacity: rowVisible.includes(i) ? 1 : 0,
                      transform: rowVisible.includes(i) ? "translateX(0)" : "translateX(-50px)",
